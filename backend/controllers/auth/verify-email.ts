...
export function verifyEmailTokenHandler(req: RequestWithAuthSession, res: express.Response, next: express.NextFunction) {
    ...
    if (req.session.verifyTokenAttempts > 3) {
      ...
      new Promise((resolve, reject) => {
        User.findOne({'emailVerifyToken.value': _data.token}, (err, user) => {
          ...
          if (!user) {
            reject(ServerMessage.error(next, 401, 'Token not found'));
          } else if (moment() > user.forgotPasswordToken.exp) {
            reject(ServerMessage.error(next, 401, 'Token has expired'));
          } else {

  ...
  if (req.query.error) {
    ...
    new Promise((resolve, reject) => {
      ...
      tokenReq.on('error', (err) => {
        ServerMessage.error(req, res, 401, 'Google authentication error. Can not get token');
        reject(err);
      });
      ...
      new Promise((resolve, reject) => {
        ...
        userDataReq.on('error', (err) => {
          ServerMessage.error(req, res, 401, 'Google authentication error. Can not get user info');
          reject(err);
        });
      }).catch((err) => {
        console.error(err);
